...
这些广告不是按年份随机抽取的总体样本，且早期年份样本很少；因此年度变化可能来自行业、公司和职位构成变化，不能解释为中国上市公司整体 AI 需求的因果趋势。此外，LLM 编码尽管做了同模型盲重测和上下文裁决，仍会受量表边界、提示词版本、文本歧义和招聘文本信息不完整的影响。v2.1 已在 60 条单审核者人工开发集上完成三轮评测并通过开发门槛，冻结提示词的三次稳定性检验、573 条全量编码、120 条同模型盲重测及全部分歧裁决均已完成，现作为本项目正式结果。
...
def determine_release_status(prompt_version: str) -> str:
    """Return the publication status for a completed output version."""
...
def verify_outputs(
    ...
    *,
    ...
    required = [
        ...
        cache_dir / "labels_cache.jsonl", cache_dir / "audit_cache.jsonl",
    ...
    elif artifact_prompt_version == "2.0.0":
        required.extend(
            [
                Path("artifacts/review/v1_v2_label_comparison.csv"),
                Path("artifacts/review/v1_v2_annual_comparison.csv"),
                Path("artifacts/review/v1_v2_summary.json"),
            ]
        )
    ...
    release_status = determine_release_status(artifact_prompt_version)
    result = {"status": "PASS", "release_status": release_status, "files_checked": len(required), "canonical_ads": 573, "matched_ads": int(matched_mask.sum()), "listed_companies": int(matches.loc[matched_mask, "stock_code"].nunique()), "unmatched_ads": int((~matched_mask).sum()), "provisional_labels": 0, "reliability_sample": 120, "prompt_version": artifact_prompt_version}
    if write_report:
        ...
        files = [
            ...
            and Path("artifacts/candidates") not in path.parents
        ...
        for item in include:
            ...
            if path.is_file(): bundle.write(path, path)
                ...
                for child in path.rglob("*"):
                    if (
                        child.is_file()
                        and ".env" not in child.name
                        and "private" not in child.parts
                        and Path("artifacts/candidates") not in child.parents
                    ):
